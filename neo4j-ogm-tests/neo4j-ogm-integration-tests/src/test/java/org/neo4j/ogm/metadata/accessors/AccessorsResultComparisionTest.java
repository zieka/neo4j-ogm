...
/*
    ...
    static void generateTestData() {
        ...
        metaData.persistentEntities().forEach(classInfo -> {
            ...
            try {
                System.out.println(classInfo.name() + "," + classInfo.identityField().getName());
            } catch(MetadataException e) {
                System.out.println(classInfo.name() + ",n/a");
            }
        });
    ...
    class ClassInfoAccessorTest {
        ...
        @DisplayName("...for singular fields on")
        @ParameterizedTest(name = "{0}")
        @CsvFileSource(resources = "/org/neo4j/ogm/metadata/accessors/singular_fields_ogm32.csv")
        void computationOfFieldsMustProduceSameResults(
            String domainClass,
            ...
            String computedName = classInfoAccessor.getIdentityField().map(FieldInfo::getName).orElse("n/a");
            assertThat(computedName).isEqualTo(identityFieldName);
        }
